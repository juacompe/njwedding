...
(function($) {
	...
	$.fn.addScroll = function( settings ) {
		...
		return this.each(function() {
			var to, cont = $(this), wrap = $(this).parent(),
				...
				ey = 0, y0, tY, tT, tY1, speed, dist, min, max;
	...
	$.fn.addHint = function(txt, settings) {
		...
		var fade = function() {
			...
			hint.stop(true, false).animate({opacity: 0}, 200, function() { $(this).hide(); });
		};
		...
		var enter = function(e) {
			...
			hint.alignTo( el ).css({opacity: 0}).show().animate({ opacity: 1 }, 200);
			...
			var imgReady = function( o ) {
				...
				if ( settings.transitions ) {
					cimg.css({ 
						...
						marginTop: scrollboxHeight(),
						preScale: isimg? settings.preScale : false,
						fit: isimg? settings.fitImage : false
					});
				} else {
